...
$action = $_GET['action'];
if ($action === 'dryfruits_products') {

    try {
        $stmt = $pdo->prepare("SELECT * FROM product_details WHERE category = 'Dryfruits' ORDER BY id DESC");
        $stmt->execute();
...
} elseif ($action === 'product_search_dryfruits') {
