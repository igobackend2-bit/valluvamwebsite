...
$action = $_GET['action'];
 if ($_GET['action'] === 'millets_products') {
    try {
        $stmt = $pdo->prepare("SELECT * FROM product_details WHERE category = 'millets' ORDER BY id DESC");
        $stmt->execute();
...
}elseif ($action === 'product_search_millets') {
