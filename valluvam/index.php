...
try {
    $stmt = $pdo->query("
        SELECT id, product_name, price, dis_price, quantity, image
...
$formatProductPrice = static function ($value) {
    if ($value === null || $value === '') {
        return '';
    }
...
$getProductDiscount = static function ($price, $salePrice) {
    $priceValue = (float) preg_replace('/[^\d.]/', '', (string) $price);
    $salePriceValue = (float) preg_replace('/[^\d.]/', '', (string) $salePrice);

    if ($priceValue <= 0 || $salePriceValue <= 0 || $salePriceValue >= $priceValue) {
        return null;
    }

    return (int) round((($priceValue - $salePriceValue) / $priceValue) * 100);
};

$getProductImagePath = static function ($imageName) {
    if (!$imageName) {
        return 'images/default.jpg';
    }

    return 'assets/uploads/' . rawurlencode((string) $imageName);
};
...
<head>
    ...
    <!-- title tag -->
    <title>india's best dry fruits & nuts,oils spices and millets.</title>
    <!-- meta tag -->
    <meta charset="UTF-8">
            ...
            <div class="row" id="product-container" style="align-items:flex-start;">
                <?php if (!empty($recentProducts)) : ?>
                    <?php foreach ($recentProducts as $product) : ?>
                        ...
                        $productName = htmlspecialchars((string) ($product['product_name'] ?? 'Product'), ENT_QUOTES, 'UTF-8');
                        $productQuantity = trim((string) ($product['quantity'] ?? ''));
                        $productImage = htmlspecialchars($getProductImagePath($product['image'] ?? ''), ENT_QUOTES, 'UTF-8');
                        $discount = $getProductDiscount($product['price'] ?? '', $product['dis_price'] ?? '');
                        ?>
                        <div class="col-md-6 col-lg-4 mb-4">
                            <div class="product">
                                <a href="productdetail.php?product=<?php echo $productId; ?>" class="img-prod">
                                    <img class="img-fluid" src="<?php echo $productImage; ?>" alt="<?php echo $productName; ?>" loading="lazy" onerror="this.onerror=null;this.src='images/default.jpg';">
                                ...
                                <div class="text py-3 pb-4 px-3 text-center">
                                    <h3>
                                        <a href="productdetail.php?product=<?php echo $productId; ?>">
                                            <?php echo $productName; ?><?php echo $productQuantity !== '' ? ' (' . htmlspecialchars($productQuantity, ENT_QUOTES, 'UTF-8') . ')' : ''; ?>
                                        </a>
                                    </h3>
                                    <div class="d-flex justify-content-center">
                                        <div class="pricing">
                                            <p class="price">
                                                <?php
                                                $salePrice = trim((string) ($product['dis_price'] ?? ''));
                                                $price = trim((string) ($product['price'] ?? ''));
                                                $hasSalePrice = $salePrice !== '' && $discount !== null;
                                                ?>
                                                <?php if ($hasSalePrice) : ?>
                                                    <span class="mr-2 price-dc">&#8377;<?php echo $formatProductPrice($price); ?></span>
                                                    <span class="price-sale">&#8377;<?php echo $formatProductPrice($salePrice); ?></span>
                                                <?php else : ?>
                                                    <span>&#8377;<?php echo $formatProductPrice($price); ?></span>
                                                <?php endif; ?>
                                            </p>
                                        </div>
                                    </div>
                                    <div class="bottom-area d-flex px-3">
                                        <div class="m-auto d-flex">
                                            <a href="productdetail.php?product=<?php echo $productId; ?>" class="add-to-cart d-flex justify-content-center align-items-center text-center" title="View Details">
                                                <span><ion-icon name="menu"></ion-icon></span>
                                            </a>
                                            <a href="#" class="buy-now btn btn-primary add-to-cart d-flex justify-content-center align-items-center" data-id="<?php echo $productId; ?>" title="Add to cart">
                                                <span><ion-icon name="cart"></ion-icon></span>
                                            </a>
                                            <a href="#" class="heart d-flex justify-content-center align-items-center wishlist-btn" data-product-id="<?php echo $productId; ?>" title="Add to wishlist">
